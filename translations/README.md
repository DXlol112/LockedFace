...
При запуске приложение читает ключ `translations_select` из пользовательского
`config.json`. Значение `RU` оставляет исходный русский интерфейс, а `EN`
загружает `translations/en/lockedface_en.qm`. Выбор языка применяется сразу,
без перезапуска приложения. Если английский каталог не найден, исходные строки
остаются на русском языке.
